...
use twilight_model::id::{marker::{MessageMarker, UserMarker}, Id};
...
impl RinfoCommand {
    pub async fn handle(
        ...
        let (channel_id, message_id) = match utils::parse_message_identifier(&self.message) {
            Ok(ids) => ids,
            Err(e) => {
                return Ok(create_error_response(&format!("Error parsing message identifier: {}", e)));
            }
        };
        ...
        let message = match http.message(channel_id, message_id).await {
            Ok(response) => match response.model().await {
                Ok(msg) => msg,
                Err(e) => return Ok(create_error_response(&format!("Error parsing message: {}", e))),
            },
            Err(e) => return Ok(create_error_response(&format!("Error fetching message: {}", e))),
        };
        ...
        let formatted_info = format_reaction_info(
            reaction_users_map,
            self.user_only.unwrap_or(false),
        );
